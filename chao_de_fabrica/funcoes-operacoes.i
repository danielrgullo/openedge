...
FUNCTION verifica-saldo-reservas RETURNS LOGICAL (nr-ord-prod AS INTEGER,
    ...
    IF NOT AVAIL b-oper-ord THEN DO:
        ...
        RETURN FALSE. /* não é para vir para cá */
    END.
    /* E se o split teve o grupo de máquina alterado? veja IF mais abaixo */
    ...
    IF cod-depos = "" OR get-grup-maquina-ctrab(cod-ctrab) <> b-oper-ord.gm-codigo THEN DO:
        ASSIGN cod-depos = deposito-ctrab(cod-ctrab). /*pode isso ? - tem casos de um centro de trabalho para mais de uma area de produção. */
    END.

    RUN ccs/op/saldo-reservas.p (INPUT 2, /* por operação */
                                 INPUT nr-ord-prod,
                                 INPUT op-codigo,
                                 INPUT qt-reportada,
                                 INPUT NO, /* não faz as requisições, só verifica */
                                 INPUT cod-depos,
